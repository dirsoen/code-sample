...
public class FormComponentExt {
    public void selectFormById() {
        ...
        if (SharedUtil.isEmptyList(results)) {
            ...
            formDetail = new FormDetail(docId, resultMap.get("name"), masterId, masterName, resultMap.get("type"), resultMap.get("typeId"), resultMap.get("isMaster"), resultMap.get("remark"));
            ...
            for (Map<String, Object> regionMap : regions) {
                ...
                Region region = new Region(regionName, regionMap.get("seqNo"), regionId, regionType, regionMap.get("isFromMaster"));
                List<Map<String, Object>> fieldMaps = jdbcTemplate.queryForList("select a.seq_no seqNo, a.is_mandatory required, b.name fieldName, b.code fieldType, c.name fieldTypeName from ap_doc_region_field a" +
                ...
                if (SharedUtil.isEmptyList(instances)) {
                    for (Map<String, Object> map : fieldMaps) {
                        Field field = new Field(map.get("fieldName"), map.get("fieldTypeName"), map.get("fieldType"), null, map.get("seqNo"), map.get("required"));
                        fields.add(field);
                    }
                } else {
                    Object instanceId = instances.get(0).get("id");
                    List<Map<String, Object>> values = jdbcTemplate.queryForList("select field_value from ap_doc_inst_field_value where ap_doc_inst_id = ? and ap_doc_region_id = ?", instanceId, regionId);
                    ...
                    for (int j = 0; j < values.size(); j++) {
                        String rows = (String) values.get(j).get("field_value");
                        Field field = new Field();
                        if (regionType == 1) {
                            String[] cols = rows.split(",");
                            for (String col : cols) {
                                field = new Field();
                                setValue(field, fieldMaps.get(j), col);
                                fields.add(field);
                            }
                        } else {
                            setValue(field, fieldMaps.get(j), rows);
                            fields.add(field);
                        }
                    }
                }
    ...
    public void updateForm() {
        ...
        jdbcTemplate.update("update ap_doc T set T.name = ?, T.remark = ?, T.ap_doc_type_id = ?, T.ap_doc_mp_id = ? where id = ?",
                formCreateModel.getFormName(), formCreateModel.getFormInstructions(), formCreateModel.getFormType(), formCreateModel.getMasterMask(), formCreateModel.getId());
        ...
        String apDocVerMBId = jdbcTemplate.queryForObject("select AP_DOC_VER.ID from AP_DOC_VER where AP_DOC_VER.AP_DOC_ID=" + formCreateModel.getMasterMask() + " order by ver_no desc limit 1", String.class);
        ...
        regionList.forEach(region -> {
            ...
            for (FieldInfoModel field : fields) {
                ...
                if (region.getRegionType() == 1) {
                    valueMap.put(field.getFieldId(), valueMap.getOrDefault(field.getFieldId(), "") + field.getValue() + ",");
                } else {
                    valueMap.put(field.getFieldId(), field.getValue());
                }
            }
